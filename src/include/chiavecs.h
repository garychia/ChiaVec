...
#define CUDAVEC_DECLARE_VECS(type)                                                                                        \
    ...
    void Vec_##type##_init_with_values(struct Vec_##type *vec, const type *values, size_t size, uint8_t on_host);         \
    void CudaVec_##type##_init_with_values(struct CudaVec_##type *vec, const type *values, size_t size, uint8_t on_host); \
